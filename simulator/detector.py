...
This is a "hopping window": WINDOW_SECONDS=10, SLIDE_SECONDS=5 means
windows overlap by 50%, so we re-check every 5s using the last 10s of
data. That overlap is what gives us faster detection than a tumbling
(non-overlapping) window, at the cost of doing ~2x the recomputation.
...
WINDOW_SECONDS = 10
SLIDE_SECONDS = 5
ERROR_RATE_THRESHOLD = 0.10  # 10% error rate triggers an incident

# Per-service rolling buffers. Each entry is (timestamp, value_dict).
metric_buffers = defaultdict(deque)
log_buffers = defaultdict(deque)

# Track which services currently have an open incident, so we don't
# spam duplicate incidents every single window while they're still sick.
open_incidents = set()
...
def trim_old_entries(buffer, service):
    """Drop anything older than WINDOW_SECONDS - this is what makes it a sliding window."""
...
def compute_window_stats(service):
    """Aggregate the current window's metrics + logs for one service."""
    ...
    error_log_count = sum(1 for l in logs if l["level"] == "ERROR")

    return {
        "avg_error_rate": round(avg_error_rate, 4),
        "avg_latency_ms": round(avg_latency_ms, 1),
        "error_log_count": error_log_count,
        "sample_log_messages": [l["message"] for l in logs if l["level"] == "ERROR"][:3],
...
def check_recovery(service, stats):
    """If a service was sick but is now back under threshold, close the incident."""
...
def evaluate_all_services(producer):
    ...
    for service in all_services:
        ...
        print(f"[WINDOW] {service}: error_rate={stats['avg_error_rate']:.3f} samples={stats['sample_count']} open={service in open_incidents}")
...
def main():
    ...
    print(f"Detector started. window={WINDOW_SECONDS}s slide={SLIDE_SECONDS}s threshold={ERROR_RATE_THRESHOLD}")
    ...
    for message in consumer:
        ...
        # Only recompute windows every SLIDE_SECONDS, not on every single message.
